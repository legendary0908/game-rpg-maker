 ...
 * @arg omitImages
 * @type string
 * @text Omitir Imágenes
 * @desc IDs de casillas a omitir, separadas por comas. Ejemplo: "145, 146, 147"
 *
 * @arg z
 * @type number
 * @min 0
...
const loadDynamicLayerImages = (layerId, x, y, mapWidth, mapHeight, imagePath, z, multiples) => {
    ...
    const stepX = step * tileWidth;
    const stepY = step * tileHeight;
    ...
    for (let i = extendedStartX, posX = i * tileWidth; i < extendedEndX; i += step, posX += stepX) {
        for (let j = extendedStartY, posY = j * tileHeight; j < extendedEndY; j += step, posY += stepY) {
            ...
            if (topLeftTile) {
                const associatedTileKey = `${topLeftTile.x},${topLeftTile.y}`;
                imageTileID = _tileIDs[_currentMapId][associatedTileKey] || 0;

                // Posición basada en la lógica de macrotiles
                originalX = topLeftTile.x * tileWidth;
                originalY = topLeftTile.y * tileHeight;
            } else {
                // En caso de que no haya macrotile, usamos la posición por defecto
                originalX = posX;
                originalY = posY;
            }

            // Verificar si el ID de la casilla está en la lista de omitidos para esta capa
            if (_OmittedTileIds[layerId] && _OmittedTileIds[layerId].includes(imageTileID)) {
                continue; // Omitir esta casilla
            }

            if (!_tileIDs[_currentMapId][layerId][imageTileID] || !_tileIDs[_currentMapId][layerId][imageTileID].sprite) {
                let sprite = getSpriteFromPool();
                if (!sprite) continue;

                if (!loadedBitmaps[imageTileID]) {
                    sprite.bitmap = ImageManager.loadBitmap(imagePath + "/", String(imageTileID));
                    loadedBitmaps[imageTileID] = true;
                }
                ...
                _tileIDs[_currentMapId][layerId][imageTileID] = { sprite: sprite };
                layerContainer.addChild(sprite);
            } else {
                let sprite = _tileIDs[_currentMapId][layerId][imageTileID].sprite;
                sprite.x = originalX;
                sprite.y = originalY;
            }
        }
...
// Procesar el parámetro omitImages
const omitImagesArray = args.omitImages ? args.omitImages.split(',').map(Number) : [];
_OmittedTileIds[id] = omitImagesArray;
